...
class NodeMinibatchIterator(object):
    ...
    def incremental_embed_feed_dict(self, size, iter_num):
        node_list = self.nodes
        val_nodes_appoggio = node_list[iter_num*size:min((iter_num+1)*size, 
            len(node_list))]
        val_nodes=[(n,n) for n in val_nodes_appoggio]
        return self.batch_feed_dict(val_nodes_appoggio), (iter_num+1)*size >= len(node_list), val_nodes
