...
flags.DEFINE_integer('batch_size', 24, 'minibatch size.')
...
flags.DEFINE_integer('validate_batch_size', 12, "how many nodes per validation sample.")
...
def evaluate(sess, model, minibatch_iter, size=None):
    ...
    node_outs_val = sess.run([model.loss,model.preds], 
                        feed_dict=feed_dict_val)
    mic, mac = calc_f1(labels, node_outs_val[1])
    return node_outs_val[0], mic, mac, (time.time() - t_test)
...
def incremental_evaluate(sess, model, minibatch_iter, size, test=False):
    ...
    while not finished:
        ...
        node_outs_val = sess.run([model.loss,model.preds],feed_dict=feed_dict_val)
        val_losses.append(node_outs_val[0])
...
def save_val_embeddings(sess, model, minibatch_iter, size, out_dir, mod=""):
    ...
    name = "val"
    while not finished:
        feed_dict_val_tutti_e_due, finished, nodes = minibatch_iter.incremental_embed_feed_dict(size, iter_num)
        iter_num += 1
        feed_dict_val=feed_dict_val_tutti_e_due[0]

        #outs = sess.run([merged, model.opt_op, model.loss, model.preds], feed_dict=feed_dict)
        print('appena prima di node_outs_val')
        node_outs_val = sess.run([ model.loss,model.preds,model.outputs1], feed_dict=feed_dict_val)
        print('appena dopo node_outs_val')
